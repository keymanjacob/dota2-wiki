...
if(isset($_GET['fbn'])&&isset($_GET['flag'])){
  $fb_name = $_GET['fbn'];
  $sql1 = "select * from user where user_name = '$fb_name';";
  $res = $db->send_sql($sql1);
  $res2 = mysql_fetch_array($res);
  if($res2){
    $_SESSION['login_name']=$res2['user_name'];
    ...
    $sql2 = "insert into user (user_name) values ('$fb_name')";
    $res3 = $db->send_sql($sql2);
    $sql3 = "commit;";
    $res4 = $db->send_sql($sql3);
    $res = $db->send_sql($sql1);
  $res2 = mysql_fetch_array($res);
    $_SESSION['login_name']=$res2['user_name'];
        $_SESSION['user_id']=$res2['user_id'];
